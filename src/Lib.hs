...
module Lib (
    conway, toMap, termsList, finite, mono, mono1, veb, veb1, omega, epsilon0,
    Conway, epsilon
) where
...
finite :: (One a, OrdZero a) => a -> Conway a
finite = mono zero

-- | w^p.c
mono :: (One a, OrdZero a) => Conway a -> a -> Conway a
mono p c = if c == one then mono1 p else conway $ M.singleton (zero, p) c

-- | w^p
mono1 :: (One a, OrdZero a) => Conway a -> Conway a
mono1 p
    | isVebFixed zero p = p
    | otherwise = conway $ M.singleton (zero, p) one
...
-- | V_a(p).c
veb :: (One a, OrdZero a) => Ordinal -> Conway a -> a -> Conway a
veb a p c
    | c == one = veb1 a p
...
multMono ((a, p), c) (Conway x) = foldr combineMono zero $ M.toList x where
    combineMono ((a', p'), c') s
        -- w^p.c * w^p'.c' = w^[p + p']. (c * c')
      | isZero a && isZero a' = add s $ mono (add p p') c''
        -- w^p.c * V_a'(p').c' = w^p.c * w^[V_a'(p)].c' = w^[p + V_a'(p)]. (c * c')
      | isZero a && not (isZero a') = add s $ mono (add p (veb1 a' p')) c''
      | not (isZero a) && isZero a' = add s $ mono (add (veb1 a p) p') c''
        -- V_a(p).c * V_a'(p').c' = w^[V_a(p)].c * w^[V_a'(p)].c' = w^[V_a(p) + V_a'(p)]. (c * c')
