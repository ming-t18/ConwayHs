...
module Seq.InfList where

import Conway (Natural, Ordinal, finiteView, omega)
import Data.List (intercalate)
import Seq.Types (Seq (..))
import Prelude hiding (iterate, length, repeat, take)
...
instance (Show a) => Show (Infinite a) where
  show xs = "Inf [" ++ intercalate ", " (Prelude.map show (take 5 xs)) ++ ", ...]"
...
instance Seq (Infinite a) Ordinal a where
  length _ = omega
